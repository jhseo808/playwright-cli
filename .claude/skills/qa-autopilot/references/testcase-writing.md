...
## playwright-cli 액션 로그 기반 TC 보강

`playwright-cli` 액션(`click`, `fill`, `snapshot`)에서 생성되는 실행 로그를 TC 문장으로 변환해 정확도를 높입니다.

### 적용 규칙

1. `snapshot`으로 확보한 요소명/역할을 TC 단계 문장에 반영
2. 액션 로그는 테스트 단계로, `expect` 계열 검증은 기대 결과로 분리
3. 자동 생성된 액션 흐름은 초안으로 사용하고, assertion은 수동 보강

### 반영 예시

- 실행 로그: `await page.getByRole('button', { name: '로그인' }).click();`
- TC 단계: `로그인 버튼을 클릭한다.`
- 기대 결과: `로그인 폼 또는 로그인 후 화면으로 정상 전환된다.`

### 품질 체크리스트

- TC 단계가 특정 element id(e1, e2)에 의존하지 않는가?
- 사용자 관점 문장(행동/결과)으로 변환되었는가?
- 예외/에러 시나리오가 최소 1개 이상 포함되었는가?
